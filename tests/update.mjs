...
let published = null; // when set, index.html really is this build
const server = createServer((req, res) => {
  ...
  if (path === '/' || path === '/index.html') { let html = readFileSync(join(ROOT, 'index.html'), 'utf8'); if (published) html = html.replace(/const BUILD_ID = '[^']*'/, `const BUILD_ID = '${published}'`).replace(/<meta name="glimmer-build" content="[^"]*">/, `<meta name="glimmer-build" content="${published}">`); res.writeHead(200, { 'content-type': 'text/html' }); return res.end(html); }
...
// version.json changed but the page didn't (an out-of-step upload): stay put
fake = 'out-of-step';
await page.waitForTimeout(15000);
ok(loads.length === 1, "doesn't reload when only version.json changed");

fake = 'test-next'; published = 'test-next';
