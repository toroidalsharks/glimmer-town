...
function renderTexts() {
  ...
  if (textThread) {
    ...
    pane.innerHTML = `<button class="back" type="button" data-thread="">&larr; All texts</button><p class="label">${esc(title)}</p><div class="chat">${msgs.slice(-60).map((m) => { const p = person(m.from); const side = textThread === 'town' ? 'l' : m.from === left ? 'l' : 'r'; return `<div class="msg ${side}${['bully', 'mean'].includes(m.tone) ? ' bad' : ''}"><span class="msg-n" style="color:${p ? skinCss(p) : 'var(--dim)'}">${esc(m.fromName)}</span><span class="msg-t">${esc(m.text)}</span><span class="msg-d">Day ${m.day} · ${clockAt(m.t)}</span></div>`; }).join('') || '<p class="hint">No texts yet.</p>'}</div>`;
