...
function workshopHtml() {
  ...
  return `<p class="label">Your workshop</p>
    ...
    <div class="field"><label for="craftColor">Color</label><select id="craftColor">${Object.keys(COLORS).map((c) => `<option>${c}</option>`).join('')}</select></div>
    <div class="items">${Object.keys(DECOR).map((id) => card('decor', id, null, DECOR[id].name)).join('')}${Object.keys(HATS).map((id) => card('hat', id, null, HATS[id].name)).join('')}</div>`;
}
let lastSongTalk = 0;
