...
function bodyFacts(p, you = true) {
  ...
  if (typeof vibeWords === 'function' && p.body) f.push(you ? `Your personality: ${vibeWords(p).join(', ')}.${p.look ? ` You dress ${FASHION[p.look.fashion].name.replace(/ \(.*\)/, '').toLowerCase()}.` : ''}` : `${vibeWords(p).join(', ')}${p.look ? `, dresses ${FASHION[p.look.fashion].name.replace(/ \(.*\)/, '').toLowerCase()}` : ''}`);
...
function outfitText(p) { if (p.look && typeof lookText === 'function' && p.body) return lookText(p) + (p.outfit.hat && !inUniform(p) ? ` and ${a_an(hatText(p.outfit.hat))}` : ''); return `${a_an(p.outfit.shirt)} shirt${p.outfit.hat ? ` and ${a_an(hatText(p.outfit.hat))}` : ''}`; }
