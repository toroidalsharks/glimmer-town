...
// Mili lost her pixie cut to a random morning haircut; put it back once (the color stays hers)
function miliHairFix() {
  W.added = W.added || {}; if (W.added.miliPixie) return; W.added.miliPixie = true;
  const m = typeof findMili === 'function' && findMili(); if (!m || !m.body) return;
  const h = hairOf(m); h.style = 'pixie'; if (typeof restyleHair === 'function') restyleHair(m);
  if (typeof logUpdate === 'function') logUpdate('build', `${m.name} has her own haircut back: a wispy layered pixie, a little longer on top and swept to one side. The random morning haircuts also leave Mili, Red, Tim and invited residents alone now, so their hair only changes when you change it.`, 'Mili\'s pixie cut');
}
function v25Boot() {
