...
function suspectRow(C, pid) {
  ...
  return `<div class="note" style="display:flex;flex-direction:column;gap:4px">
    ...
    <span class="hint" style="font-size:12px">${swatch(COLORS[T.shirt] || '#ccc', T.shirt)}${esc(T.shirt)} top · ${swatch(HAIR_COLORS[T.hair] || '#555', T.hair)}${esc(T.hair)} hair · ${swatch(SHOE_HEX[T.shoe] || '#fff', T.shoe)}${esc(T.shoe)} shoes · ${esc(T.build)} · ${esc(T.ears === 'none' ? 'no ears' : T.ears + ' ears')}</span></div>
    <div class="hint" style="font-size:12px">${esc(C.motives[pid] || '')}</div>
...
function crimeCard(C) {
  ...
  return `<div class="creator"><h3>${K.icon} ${esc(K.label)}${C.victimName ? ` · ${esc(C.victimName)}` : ''}</h3>
    <div class="chips"><span class="chip">case #${C.n}</span><span class="chip">day ${C.day}</span>${status}</div>
    <p>${esc(C.headline)}</p>
    <p class="label" style="margin-top:6px">Clues (${found.length}${left ? `, ${left} still out there` : ', all found'})</p>
    ${found.map((c) => { const f = fitsClue(C, c); return `<p class="note">${c.icon || '•'} ${esc(c.text)}${f.length && !c.clears ? ` <span class="hint">fits: ${f.map((id) => esc(nameOf(id))).join(', ')}</span>` : c.clears ? ` <span class="hint">clears ${esc(nameOf(c.pid))}</span>` : ''}</p>`; }).join('') || '<p class="hint">Nothing yet.</p>'}
    ${C.status === 'open' ? `<div class="btns"><button class="btn" type="button" data-crime="search" data-cid="${C.id}" ${C.searchDay === W.day || !left ? 'disabled' : ''}>🔍 Search the scene${C.searchDay === W.day ? ' (tomorrow)' : ''}</button></div>` : ''}
...
In one or two short sentences, answer the Creator in your own voice. Reply with only JSON: {"say":"..."}`, { max: 160, temperature: 0.9 });
      if (r?.say) q.said = String(r.say).slice(0, 220);
    } catch (e) {}
  }
