...
// ============================================================
const HAIR_STYLES = { short: 'Short', bob: 'Bob', long: 'Long', spiky: 'Spiky', bun: 'Bun', twintails: 'Pigtails', curly: 'Curly', buzz: 'Buzz', none: 'None' };
...
const SHOE_COLORS = ['#ff6f5e', '#3d4f86', '#fffaf2', '#ffb347', '#8a84a8', '#5a3a2e', '#ff9fbf', '#6fbf6a'];
...
function hairOf(p) {
  ...
  if (!p.body.hair || !HAIR_STYLES[p.body.hair.style]) {
    ...
    if (typeof isMili === 'function' && isMili(p)) p.body.hair = { style: 'bob', color: 'ink', shoe: '#fffaf2' };
...
function faceUpdate(p, m, blink) {
  ...
  m.eyes.forEach((e) => { e.visible = !laugh; e.scale.y = blink ? 0.2 : expr === 'sad' ? 1.05 : expr === 'surprised' ? 1.7 : expr === 'angry' ? 1.2 : 1.45; });
  m.happyEyes.forEach((a) => (a.visible = laugh));
  m.shines.forEach((s) => (s.visible = !blink && !laugh));
  const tilt = expr === 'angry' ? 0.38 : expr === 'sad' ? -0.32 : 0;
  m.brows.forEach((b) => { b.rotation.z = b.userData.s * tilt; b.position.y = expr === 'surprised' ? 0.3 : expr === 'sad' ? 0.24 : 0.25; });
  ...
  m.mouth.visible = shape === 'smile'; m.mouthFrown.visible = shape === 'frown'; m.mouthO.visible = shape === 'o'; m.mouthOpen.visible = shape === 'open';
  if (shape === 'open') m.mouthOpen.scale.y = talking ? 0.45 + talkK * 0.55 : 0.8;
  if (shape === 'smile') m.mouth.scale.set(expr === 'happy' ? 1.25 : 1, expr === 'happy' ? 1.2 : 1, 1);
}
