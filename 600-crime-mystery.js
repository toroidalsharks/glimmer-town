...
const jailed = (p) => !!(p && p.jail && W.day < p.jail.until);
...
const SHOE_NAMES = { '#ff6f5e': 'red', '#3d4f86': 'navy', '#fffaf2': 'white', '#ffb347': 'orange', '#8a84a8': 'gray', '#5a3a2e': 'brown', '#ff9fbf': 'pink', '#6fbf6a': 'green' };
function traitsOf(p) {
  const h = typeof hairOf === 'function' ? hairOf(p) : { color: 'cocoa', shoe: '#fffaf2' };
  return { shirt: p.outfit?.shirt || 'cream', hair: h.color, shoe: SHOE_NAMES[h.shoe] || 'white', build: p.body.size >= 1.07 ? 'tall' : p.body.size <= 0.95 ? 'small' : 'medium', ears: p.body.ears || 'none', job: p.job || null, laptop: hasLaptop(p) };
}
const TRAIT_WORD = {
...
const fitsClue = (C, c) => C.suspects.filter((pid) => (c.pid ? c.pid === pid && !c.clears : c.attr && C.traits[pid]?.[c.attr] === c.val));
...
function crimeNight() {
  ...
  if (!MY.done && W.day >= MY.day && W.people.filter(crimeAble).length >= 6 && !openCrimes().some((C) => C.type === 'murder')) { if (commitMurder()) MY.done = true; }
...
function crimeMorning() {
  ...
  // releases
  for (const p of W.people) if (p.jail && W.day >= p.jail.until) releaseFromJail(p, 'served');
  // funerals
...
function crimeLines(C, T) {
  ...
  L.push(pline(pr, pick([`Your honor, ${C.headline} The evidence points to one person, and they are sitting right there.`, `On day ${C.day}, ${C.headline.charAt(0).toLowerCase() + C.headline.slice(1)} I will show the court that ${A.name} did it.`])));
  L.push(pline(co, pick([`My client didn't do this. This whole case is gossip wearing a fancy hat.`, `${A.name} is innocent, and the prosecution knows its evidence is thin.`, `The real culprit is still out there, laughing at all of us.`])));
  ...
  L.push(pline(co, pick([`Everything you heard today is a guess. You can't lock someone up for a guess.`, `If you're not sure, you have to let ${A.name} go. That is the rule.`])));
  L.push(pline(pr, pick([`The clues all point the same way. ${A.name} did this.`, `Look at the evidence, not the tears.`])));
  return L;
}
async function aiCrimeLines(C, T) {
  if (!aiReady() || aiBusy >= 3) return null;
  const A = person(T.def), K = CRIME_TYPES[C.type]; if (!A) return null;
  const cast = [T.pros, T.counsel, T.def, T.witness].map(person).filter(Boolean);
  const found = C.clues.filter((c) => c.found);
  try {
    const r = await llm(`A courtroom scene in Glimmer Hall on ${ISL.name}, a tiny island town of cute villagers who are real, flawed people. Tone: a dramatic movie trial, tense, with a few funny beats. The judge is Judge Hoot, a fussy old owl who says "hoo".
THE CHARGE: ${A.name} is accused of ${K.label.toLowerCase()}. ${C.headline}${C.type === 'murder' ? ' Keep it non-graphic: no gore, no weapons detail.' : ''}
PROSECUTOR: ${person(T.pros)?.name}. DEFENSE: ${T.counsel ? person(T.counsel)?.name : `${A.name}, representing themself`}. WITNESS: ${person(T.witness)?.name || 'none'}.
THE DEFENDANT: ${sceneProfile(A, { members: [] })}. Their alibi: "${C.alibis[A.id]}". ${C.culprit === A.id ? `SECRET THE SCRIPT KNOWS: ${A.name} really did it (${A.name} ${C.why}). They lie and squirm, and may crack.` : `SECRET THE SCRIPT KNOWS: ${A.name} is innocent. They are scared and angry.`}
EVIDENCE FOUND SO FAR (use these exact facts, do not invent new evidence):
${found.map((c, i) => `${i + 1}. ${c.text}`).join('\n')}
OTHER SUSPECTS: ${C.suspects.filter((id) => id !== A.id).map(nameOf).join(', ') || 'none'}
Write 18 to 26 lines of the trial, BEFORE the verdict: openings, the witness on the stand, cross-examination, at least three objections ("Objection! …" with a legal-sounding reason) that Judge Hoot sustains or overrules, the prosecution presenting evidence ("Take that!" once), the defense pointing at another suspect, the defendant testifying, one "Hold it!" moment on a contradiction, gallery gasps, and closings. Keep every line under 26 words. Stop before any verdict.
Reply with only JSON: {"lines":[{"who":"name, or JUDGE, or GALLERY","say":"...","fx":"objection, holdit, takethat, gasp, gavel, shock or none"}]}`, { model: badModels.has(brainCfg.judge) ? null : brainCfg.judge, max: 1600, temperature: 0.9 });
    return aiToLines(r?.lines, cast);
  } catch (e) { return null; }
}
function crimeVerdict(C, T, k, silent) {
  ...
  if (guilty) {
    const days = K.days, fine = Math.min(A.coins, (C.loss || 0) + K.fine);
    ...
    if (days > 0) { A.jail = { until: W.day + days, crime: C.id, label: K.label }; }
    else if (typeof autoSpot === 'function') { const sp = autoSpot(); if (sp) setTask(A, 'service', 'plaza', [sp[0] + 1.2, sp[1]], { x: sp[0], z: sp[1] }); }
    C.sentence = days > 0 ? `${days} days in the cell under Glimmer Hall${fine ? ` and ${fine} coins in restitution` : ''}` : `${fine} coins in restitution and community service`;
    L.push(jline(`${A.name}, the court sentences you to ${C.sentence}. Hoo.`, 'gavel'));
    ...
    for (const q of W.people) if (q !== A && adult(q)) { remember(q, `${A.name} was found guilty of ${K.label.toLowerCase()} and got ${C.sentence}.`, 2, 'verdict', A.name); if (fscore(q, A) > 2 && rand() < 0.5) feel(q, A, -1.5, true); }
    remember(A, `I was found guilty of ${K.label.toLowerCase()}. ${C.wrong ? 'I DIDN\'T DO IT.' : 'It is over.'}`, 4, 'convicted');
    ...
    if (days > 0) { A.task = { kind: 'jail', phase: 'do' }; }
  } else {
    ...
    C.status = C.clues.some((c) => !c.found) ? 'open' : 'cold'; C.accused = null; C.acquitted = [...(C.acquitted || []), A.id];
  ...
  diary(`⚖ Verdict in <b>The Town v. ${esc(A.name)}</b> (${esc(K.label.toLowerCase())})${C.jury ? `, jury ${C.jury}` : C.byCreator ? ', ruled by the Creator' : ''}: <b>${guilty ? 'GUILTY' : 'NOT GUILTY'}</b>${guilty ? `. ${esc(C.sentence)}.` : '.'}`);
...
function releaseFromJail(p, why) {
  ...
  diary(`🔓 <b>${esc(p.name)}</b> ${why === 'pardon' ? 'was pardoned by the Creator' : why === 'exonerated' ? 'was set free. They were innocent all along' : 'finished their sentence'} and walked out of Glimmer Hall.`);
  remember(p, why === 'pardon' ? 'The Creator pardoned me. I am free.' : why === 'exonerated' ? 'They finally found out it wasn\'t me. I lost days of my life for nothing.' : 'I served my time. I am out.', 3, 'released');
  if (why !== 'served' || (C && C.type === 'murder')) releaseCut(p, why, C);
  markDirty();
}
